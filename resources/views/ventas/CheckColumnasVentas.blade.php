<div class="d-flex justify-content-between align-items-start">

    <div class="row">
        
        <div class="col-2">

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="0" id="colFactura" checked>
                <label class="form-check-label" for="colFactura">Factura</label>
            </div>

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="1" id="colCliente" checked>
                <label class="form-check-label" for="colCliente">Cliente</label>
            </div>
            
        </div> <!-- FIN DE COLUMNA -->

        <div class="col-2">

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="2" id="colUsuario" checked>
                <label class="form-check-label" for="colUsuario">Usuario</label>
            </div>

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="3" id="colFecha" checked>
                <label class="form-check-label" for="colFecha">Fecha</label>
            </div>

        </div> <!-- FIN DE COLUMNA -->

        <div class="col-2">

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="4" id="colSubtotal" checked>
                <label class="form-check-label" for="colSubtotal">Subtotal</label>
            </div>
            
            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="5" id="colImpuesto" checked>
                <label class="form-check-label" for="colImpuesto">Impuesto</label>
            </div>

        </div> <!-- FIN DE COLUMNA -->

        <div class="col-2">

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="6" id="colTotal" checked>
                <label class="form-check-label" for="colTotal">Total</label>
            </div>

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="7" id="colMetodoPago">
                <label class="form-check-label" for="colMetodoPago">Método Pago</label>
            </div>

        </div> <!-- FIN DE COLUMNA -->

        <div class="col-2">

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="8" id="colEstado">
                <label class="form-check-label" for="colEstado">Estado</label>
            </div>

            <div class="form-check">
                <input class="form-check-input toggle-col" type="checkbox" data-column="9" id="colDetalles" checked>
                <label class="form-check-label" for="colDetalles">Detalles</label>
            </div>
            
        </div> <!-- FIN DE COLUMNA -->

    </div>
    
</div>